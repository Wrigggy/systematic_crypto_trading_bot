...
1. Data Engine (/data)
connector.py: Handles asynchronous WebSocket connections to Web3 exchanges. Includes auto-reconnect logic and heartbeats.

buffer.py: A thread-safe, in-memory sliding window (Buffer) that stores the most recent N ticks/K-lines for real-time feature calculation.

2. Alpha & Feature Engine (/features & /models)
extractor.py: Transforms raw OHLCV data into technical indicators and non-linear features (e.g., Volatility, Order Flow Imbalance).

model_wrapper.py: Loads pre-trained Deep Learning models (ONNX, PyTorch, or Scikit-learn).

inference.py: Runs real-time forward passes to generate Alpha scores (expected return or buy/sell probabilities).

3. Strategy & Monitor (/strategy)
monitor.py: The central event loop. It listens to data updates and triggers the Alpha engine.

logic.py: Implements the "Decision State Machine."
