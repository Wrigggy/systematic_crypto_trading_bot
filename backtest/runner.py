...
from backtest.onchain_loader import load_onchain_datasets
...
class BacktestRunner:
    ...
    async def run(self, datasets: Dict[str, SymbolDataset], run_id: str) -> BacktestResult:
        ...
        onchain_datasets = load_onchain_datasets(
            list(datasets),
            start=min(dataset.frame.iloc[0]["timestamp"].to_pydatetime() for dataset in datasets.values()),
            end=max(dataset.frame.iloc[-1]["timestamp"].to_pydatetime() for dataset in datasets.values()),
        )
        onchain_state = {symbol: {"index": 0, "latest": None} for symbol in onchain_datasets}
        ...
        for iteration, (timestamp, candles) in enumerate(iter_candle_batches(datasets), start=1):
            ...
            for candle in candles:
                ...
                onchain_dataset = onchain_datasets.get(candle.symbol)
                if onchain_dataset is None:
                    continue
                state = onchain_state[candle.symbol]
                frame = onchain_dataset.frame
                while state["index"] < len(frame):
                    row = frame.iloc[state["index"]]
                    row_ts = row["timestamp"].to_pydatetime()
                    if row_ts > timestamp:
                        break
                    state["latest"] = row
                    state["index"] += 1
                ...
                if latest is None:
                    continue
                await buffer.push_onchain_metrics(
                    candle.symbol,
                    {key: latest.get(key) for key in ONCHAIN_METRIC_KEYS},
                )
